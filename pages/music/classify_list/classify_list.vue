		...
		<view class="menulist">
			<view class="item flex-between">
				<view class="imgbox">
					...
					<image v-else src="http://m.dance-one.com/static/default_music.png" mode="aspectFill"></image>
				</view>
			...
			<view class="item flex-between" v-for="(item,index) in datalist" :key="index" @click="toplaylist(item.Id,index)">
				...
				<view class="info flex1 flex-between">
					...
					<view class="icons flex-end">
						<view class="icon" @click.stop="playBtn(index,item.Id,item.IsShowBuy)"><image :src="(playID==item.Id&&playIDtype==1)?'http://m.dance-one.com/static/play3.png':'http://m.dance-one.com/static/play2.png'" mode="widthFix"></image></view>
